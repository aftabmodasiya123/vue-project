            ...
            <v-row no-gutters class="ma-0 pa-0">
                <v-col>
                    <v-data-table :headers="headers"
                        ...
                        <template v-slot:top>
                            <v-toolbar flat>
                                <v-toolbar-title>SECURITY TIMES</v-toolbar-title>
                                            ...
                                            <v-container>
                                                <v-row>
                                                    <v-col cols="12"
                                                           sm="6"
                                                           md="4">
                                                        <v-text-field v-model="editedItem.fields.week"
                                                                      label="Day of Week"></v-text-field>
                                                    </v-col>
                                                    <v-col cols="12"
                                                           sm="6"
                                                           md="4">
                                                        <v-text-field v-model="editedItem.fields.from_time"
                                                                      label="From Time (HH:MM)"></v-text-field>
                                                    </v-col>
                                                    <v-col cols="12"
                                                           sm="6"
                                                           md="4">
                                                        <v-text-field v-model="editedItem.fields.to_time"
                                                                      label="To Time (HH:MM)"></v-text-field>
                                                    </v-col>
                                                    <v-col cols="12"
                                                           sm="6"
                                                           md="4">
                                                        <v-text-field v-model="editedItem.fields.time"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getSecTimes(id) {
                try {
                    this.dataAccess.getSecTimes(id).then((response) => {
                        this.items = [];
                        for (var i in response.data) {
            ...
            setSecTimes(id, bag) {
                try {
                    this.errorMessages = {};
                    this.dataAccess.setSecTimes(id, bag).then(() => {
                        this.getSecTimes(id);
            ...
            deleteSecTimes(id, pk) {
                try {
                    this.dataAccess.deleteSecTimes(id, pk).then(() => {
                        this.getSecTimes(id);
                    });
                } catch (error) {
                    this.items = [];
            ...
            deleteItemConfirm() {
                this.deleteSecTimes('sec', this.items[this.editedIndex]['pk'])
            ...
            save() {
                ...
                this.setSecTimes('sec', this.editedItem);
            },
            formatHub(column) {
