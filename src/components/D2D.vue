    ...
    <v-container grid-list-md text-xs-center>
      <v-row no-gutters class="ma-0 pa-0">
        <v-col cols="3" sm="3">
          ...
          <div v-else>
            <a @click="$router.go(-1)"><strong>BACK</strong></a> /
            <strong>{{ formatFlight(flight) }}</strong>
          </div>
        </v-col>
        <v-spacer></v-spacer>
        <v-col cols="8" sm="5">
            ...
            <v-card-text class="caption active">
              <v-row no-gutters class="justify-space-around ma-0 pa-0">
                <v-col md="6">
                  ...
                  <strong
                    >{{ calculateRisk().flights }} /
                    {{ calculateRisk().pax }}</strong
                  >
                </v-col>
                <v-col md="6">
                  <v-tooltip bottom>
                  ...
                  Will Missing Connection
                  <strong
                    >{{ calculateMiss().flights }} /
                    {{ calculateMiss().pax }}</strong
                  >
                </v-col>
              </v-row>
            </v-card-text>
            ...
            >
              ...
              >
                <v-tooltip bottom v-bind:key="index">
                  <template v-slot:activator="{ on }">
                    <span v-if="header && header.text" v-on="on">{{
                      header.text
                    }}</span>
                  </template>
              ...
              <template #item="{ item }">
                <tr v-if="!isMobile">
                  ...
                  <td
                    :class="
                      darkMode == true
                        ? 'style-principal-dark text-caption'
                        : 'style-principal text-caption'
                    "
                    align="center"
                  >
                    {{ item.hub }}
                  ...
                  <td
                    ...
                    align="center"
                  >
                    <v-icon medium class="ma-0 pa-0" :color="formatPT(item)">
                      {{ formatIcon(formatPT(item)) }}
                    </v-icon>
                  </td>
                  <td class="text-caption" align="left">
                    <router-link :to="formatHRefOutbound(item)">{{
                  ...
                  <td class="text-caption" align="center">
                    {{ formatDate(item.etcg) }}
                  </td>
                  <td class="text-caption" align="center">
                    {{ formatBTD(item) }}
                  </td>
                  <td class="text-caption" align="right">
                    {{ item.business }}
                  </td>
                ...
                <tr v-else>
                  <td>
                    ...
                    >
                      <v-col cols="4" class="mx-0 my-1 pa-0">
                        ...
                        <v-list-item-title
                           class="flex-content text-caption"
                            >{{ formatFlightInbound(item) }}</v-list-item-title>
                      </v-col>
                      <v-col cols="4" class="mx-0 my-1 pa-0">
                        <v-list-item-subtitle
                          class="flex-header text-caption"
                      ...
                      <v-col cols="4" class="mx-0 my-1 pa-0">
                        <v-list-item-subtitle class="flex-header text-caption"
                          >Connectivity</v-list-item-subtitle
                        >
    ...
    try {
        this.dataAccess.getRules(id, hub).then((response) => {
            if (response.data && response.data.length > 0) {
                ...
                console.log("No rules data found for ID: " + id);
        ...
        console.error("Error while fetching rules data:", error);
        this.resetRules();
    }
},
resetRules() {
    ...
    formatBTA(item) {
      ...
      if (item.ieta != dateNull && item.ieta != item.ista)
        return util.formatDayHours(item.ieta) + "E";
      return util.formatDayHours(item.ista);
    },
    formatBTD(item) {
      if (item.ostd == null) return "";
      if (item.oatd != dateNull) return util.formatDayHours(item.oatd) + "A";
      if (item.oetd != dateNull && item.oetd != item.ostd)
        return util.formatDayHours(item.oetd) + "E";
    ...
    formatFlight(flight) {
      if (flight.operator != null && flight.operator != "") {
        return (
          util.formatFlight(flight.operator, flight.flt_nr) +
          " " +
          flight.to_iata +
          " " +
          util.formatDate(flight.std) +
          " " +
          this.formatBTD(flight)
        );
      } else {
        return "";
      }
